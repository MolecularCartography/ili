...
define([
    'utils', 'spotscontrollerbase', 'workspacebase', 'settingscontrollerbase'
],
function(Utils, SpotsControllerBase, WorkspaceBase, SettingsControllerBase) {
    function MapSelectorBase(workspace, div, mapName) {
        this._workspace = workspace;
        this._spotsController = workspace._spotsController;
        this._settingsController = workspace._settingsController;
        this._div = div;
    ...
    MapSelectorBase.prototype = Object.create(null, {
        ...
        _onWorkspaceSettingsChange: {
            value: function () {
                ...
                var mapName = this._workspace.loadedSettings[SettingsControllerBase.SETTINGS_KEYS.SELECTED_MAP];
        ...
        _onKeyPress: {
            value: function(event) {
                ...
                switch (key) {
                    case '38': // ArrowUp
                        this.navigate(MapSelectorBase.Direction.UP);
                        break;

                    case '40': // ArrowDown
                        this.navigate(MapSelectorBase.Direction.DOWN);
                        break;

                    case '33': // PageUp
                        this.navigate(MapSelectorBase.Direction.PAGE_UP);
                        break;

                    case '34': // PageDown
                        this.navigate(MapSelectorBase.Direction.PAGE_DOWN);
                        break;
        ...
        navigate: {
            value: function(direction) {
                switch (direction) {
                    ...
                    case MapSelectorBase.Direction.PAGE_DOWN:
                        var len = Math.max(10, this._itemsContainer.childElementCount / 10);
                        if (!len) return;
                        var item = this.selectedItem;
